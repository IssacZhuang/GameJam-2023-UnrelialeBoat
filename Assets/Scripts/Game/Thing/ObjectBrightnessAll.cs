...
public class ObjectBrightnessAll : BaseThing<ObjectBrightnessAllConfig>
{
    ...
    private List<InteractiveObject> interactiveObjects; // 所有子物体
    ...
    private bool isDiscoverAll = false;
    ...
    public override void OnTick()
    {
        ...
        if (!isDiscoverAll)
        {
            bool _isDiscoverAll = true;
            foreach (var interactiveObject in interactiveObjects)
            {
                if (!interactiveObject.IsDone)
                {
                    _isDiscoverAll = false;
                    break;
                }
            }
    ...
    public List<InteractiveObject> CollectObjects()
    {
        // 初始化存储材质的数组
        interactiveObjects = new List<InteractiveObject>();

        // 调用递归函数，收集材质
        CollectObjectsRecursive(this.Instance.gameObject.transform);

        return interactiveObjects;
    }

    private void CollectObjectsRecursive(Transform parent)
    {
        // 获取当前对象的渲染器组件
        InteractiveObjectBehavior behavior = parent.GetComponent<InteractiveObjectBehavior>();

        // 如果渲染器存在，收集其材质
        if (behavior != null)
        {
            if (behavior.Entity is InteractiveObject interObj)
            {
                // 将当前对象的材质添加到存储数组中
                interactiveObjects.Add(interObj);
            }
        }
